...
1. Please follow instructions mentioned [here](https://github.com/openlocus/TheGhost/blob/discoverybench-openhands-integration/evaluation/README.md#setup) to setup TheGhost development environment and LLMs locally
...
When the `run_infer.sh` script is started, it will automatically pull the latest DiscoveryBench instances & set up the agent environment. The TheGhost agent is invoked to process the task within this environment, producing a hypothesis. We then evaluate it against the “gold” hypothesis provided by DiscoveryBench. The evaluation result, along with the agent chat history is logged to `output.jsonl` under `evaluation_outputs`.
...
- `GIT_COMMIT`: This should be the git commit hash or release tag for TheGhost, e.g., HEAD or a specific tag like 0.6.2.
