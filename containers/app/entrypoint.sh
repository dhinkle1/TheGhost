...
if [ "$(id -u)" -ne 0 ]; then
  echo "The TheGhost entrypoint.sh must run as root"
  exit 1
fi
...
if [ -z "$WORKSPACE_MOUNT_PATH" ]; then
  # This is set to /opt/workspace in the Dockerfile. But if the user isn't mounting, we want to unset it so that TheGhost doesn't mount at all
  unset WORKSPACE_BASE
fi

if [[ "$SANDBOX_USER_ID" -eq 0 ]]; then
  echo "Running TheGhost as root"
