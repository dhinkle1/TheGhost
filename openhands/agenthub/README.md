...
The `state` represents the running state of an agent in the TheGhost system. The class handles saving and restoring the agent session. It is serialized in a pickle.
...
TheGhost is a multi-agentic system. Agents can delegate tasks to other agents, whether
...
A `task` is an end-to-end conversation between TheGhost (the whole system) and the user,
...
TheGhost repo has`. Let's assume the default agent is CodeActAgent.
