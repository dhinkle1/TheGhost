...
TheGhost provides a Graphical User Interface (GUI) mode for interacting with the AI assistant.
...
TheGhost supports multiple version control providers. You can configure tokens for multiple providers simultaneously.

#### GitHub Token Setup

TheGhost automatically exports a `GITHUB_TOKEN` to the shell environment if provided:
...
TheGhost automatically exports a `GITLAB_TOKEN` to the shell environment if provided:
...
Remember, the GUI mode of TheGhost is designed to make your interaction with the AI assistant as smooth and intuitive
