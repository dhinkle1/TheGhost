...
A system with a modern processor and a minimum of **4GB RAM** is recommended to run TheGhost.
...
The easiest way to run TheGhost is in Docker.
...
You'll find TheGhost running at http://localhost:3000!
...
After launching TheGhost, you **must** select an `LLM Provider` and `LLM Model` and enter a corresponding `API Key`.
...
TheGhost requires an API key to access most language models. Here's how to get an API key from the recommended providers:
...
Now you're ready to [get started with TheGhost](./getting-started).

## Versions

The [docker command above](./installation#start-the-app) pulls the most recent stable release of TheGhost. You have other options as well:
...
For the development workflow, see [Development.md](https://github.com/All-Hands-AI/TheGhost/blob/main/Development.md).
