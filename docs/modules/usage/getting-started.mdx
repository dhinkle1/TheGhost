# Getting Started with TheGhost

So you've [run TheGhost](./installation) and have
[set up your LLM](./installation#setup). Now what?

TheGhost can assist with a range of engineering tasks. However, the technology is still new, and we’re far from having
...
TheGhost is great at adding new code to an existing codebase.

For instance, you can ask TheGhost to add a GitHub action that lints your code. It might check your codebase to
...
Some tasks need more context. While TheGhost can use commands like ls and grep to search, providing context upfront
...
TheGhost does great at refactoring code in small chunks. Rather than rearchitecting the entire codebase,
...
TheGhost can help track down and fix bugs, but bug fixing can be tricky and often requires more context.
It’s helpful if you’ve already diagnosed the issue and just need TheGhost to handle the logic.
...
TheGhost can assist with nearly any coding task, but it takes some practice to get the best results.
...
See [Prompting Best Practices](./prompting/prompting-best-practices) for more tips on how to get the most out of TheGhost.
