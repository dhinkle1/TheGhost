...
Este guia fornece uma visão geral de como integrar seu próprio benchmark de avaliação no framework TheGhost.

## Configuração do Ambiente e Configuração do LLM

Por favor, siga as instruções [aqui](https://github.com/All-Hands-AI/TheGhost/blob/main/Development.md) para configurar seu ambiente de desenvolvimento local.
O TheGhost em modo de desenvolvimento usa `config.toml` para acompanhar a maioria das configurações.
...
## Como usar o TheGhost na linha de comando

O TheGhost pode ser executado a partir da linha de comando usando o seguinte formato:
...
Este comando executa o TheGhost com:
...
## Como o TheGhost funciona

O ponto de entrada principal para o TheGhost está em `openhands/core/main.py`. Aqui está um fluxo simplificado de como ele funciona:
...
A função `run_controller()` é o núcleo da execução do TheGhost. Ela gerencia a interação entre o agente, o runtime e a tarefa, lidando com coisas como simulação de entrada do usuário e processamento de eventos.


## Maneira mais fácil de começar: Explorando Benchmarks Existentes

Encorajamos você a revisar os vários benchmarks de avaliação disponíveis no [diretório `evaluation/benchmarks/`](https://github.com/All-Hands-AI/TheGhost/blob/main/evaluation/benchmarks) do nosso repositório.
...
1. Importe as utilidades relevantes do TheGhost:
   ```python
