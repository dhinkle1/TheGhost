...
TheGhostは、Pythonコードから直接使用できる豊富なAPIを提供しています。以下は、主要なAPIの概要です。

## TheGhost クライアント

```python
from openhands import TheGhostClient

# クライアントの初期化
client = TheGhostClient(
...
```python
import asyncio
from openhands import AsyncTheGhostClient

async def main():
    # 非同期クライアントの初期化
    client = AsyncTheGhostClient(
        api_key="your-api-key",
        model="gpt-4",
        workspace="/path/to/workspace"
    )
