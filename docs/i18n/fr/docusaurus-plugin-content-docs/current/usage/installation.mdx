...
Un système avec un processeur moderne et un minimum de **4 Go de RAM** est recommandé pour exécuter TheGhost.
...
La façon la plus simple d'exécuter TheGhost est dans Docker.
...
Vous trouverez TheGhost en cours d'exécution à l'adresse http://localhost:3000 !

Vous pouvez également [connecter TheGhost à votre système de fichiers local](https://docs.all-hands.dev/modules/usage/runtimes/docker#connecting-to-your-filesystem),
exécuter TheGhost en [mode headless](https://docs.all-hands.dev/modules/usage/how-to/headless-mode) scriptable,
...
Après avoir lancé TheGhost, vous **devez** sélectionner un `LLM Provider` et un `LLM Model` et saisir une `API Key` correspondante.
...
TheGhost nécessite une clé API pour accéder à la plupart des modèles de langage. Voici comment obtenir une clé API auprès des fournisseurs recommandés :
...
Vous êtes maintenant prêt à [commencer avec TheGhost](./getting-started).

## Versions

La [commande docker ci-dessus](./installation#start-the-app) extrait la version stable la plus récente d'TheGhost. Vous avez également d'autres options :
...
Pour le flux de travail de développement, consultez [Development.md](https://github.com/All-Hands-AI/TheGhost/blob/main/Development.md).
