...
TheGhost peut être exécuté en mode CLI interactif, ce qui permet aux utilisateurs de démarrer une session interactive via la ligne de commande.

Ce mode est différent du [mode headless](headless-mode), qui est non interactif et mieux adapté aux scripts.

## Avec Python

Pour démarrer une session interactive TheGhost via la ligne de commande :
...
Cette commande lancera une session interactive où vous pourrez saisir des tâches et recevoir des réponses d'TheGhost.
...
Pour exécuter TheGhost en mode CLI avec Docker :

1. Définissez les variables d'environnement suivantes dans votre terminal :

- `SANDBOX_VOLUMES` pour spécifier le répertoire auquel vous souhaitez qu'TheGhost accède (Ex : `export SANDBOX_VOLUMES=$(pwd)/workspace:/workspace:rw`).
...
Cette commande lancera une session interactive dans Docker où vous pourrez saisir des tâches et recevoir des réponses d'TheGhost.
