# Premiers pas avec TheGhost

Vous avez [exécuté TheGhost](./installation) et vous avez
[configuré votre LLM](./installation#setup). Et maintenant ?

TheGhost peut vous aider pour diverses tâches d'ingénierie. Cependant, la technologie est encore nouvelle, et nous sommes loin d'avoir
...
TheGhost est excellent pour ajouter du nouveau code à une base de code existante.

Par exemple, vous pouvez demander à TheGhost d'ajouter une action GitHub qui vérifie votre code. Il pourrait vérifier votre base de code pour
...
Certaines tâches nécessitent plus de contexte. Bien qu'TheGhost puisse utiliser des commandes comme ls et grep pour rechercher, fournir du contexte dès le départ
...
TheGhost est très efficace pour refactoriser du code en petits morceaux. Plutôt que de réarchitecturer l'ensemble de la base de code,
...
TheGhost peut aider à traquer et corriger des bugs, mais la correction de bugs peut être délicate et nécessite souvent plus de contexte.
C'est utile si vous avez déjà diagnostiqué le problème et avez juste besoin qu'TheGhost gère la logique.
...
TheGhost peut vous aider pour presque n'importe quelle tâche de codage, mais il faut un peu de pratique pour obtenir les meilleurs résultats.
...
Consultez [Meilleures pratiques de prompt](./prompting/prompting-best-practices) pour plus de conseils sur la façon de tirer le meilleur parti d'TheGhost.
