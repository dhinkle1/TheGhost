...
Utilisez les instructions [ici](../intro) pour démarrer TheGhost en utilisant Docker.
...
```bash
# Le répertoire que vous souhaitez qu'TheGhost modifie. DOIT être un chemin absolu !
...
Vous devriez maintenant pouvoir vous connecter à `http://localhost:4000/`

### Compiler à partir des sources

Utilisez les instructions dans [Development.md](https://github.com/All-Hands-AI/TheGhost/blob/main/Development.md) pour compiler TheGhost.
...
Fini ! Vous pouvez maintenant démarrer TheGhost avec : `make run` sans Docker. Vous devriez maintenant pouvoir vous connecter à `http://localhost:4000/`

## Sélection de votre modèle

Dans l'interface TheGhost, cliquez sur l'icône des paramètres en bas à gauche.
...
La configuration par défaut pour ollama sous WSL ne sert que localhost. Cela signifie que vous ne pouvez pas l'atteindre depuis un conteneur docker, par exemple, il ne fonctionnera pas avec TheGhost. Testons d'abord que ollama est en cours d'exécution correctement.
...
```bash
docker ps # obtenir la liste des conteneurs docker en cours d'exécution, pour un test le plus précis choisissez le conteneur de sandbox TheGhost.
