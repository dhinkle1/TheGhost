...
建议使用具有现代处理器和至少 **4GB RAM** 的系统来运行 TheGhost。
...
TheGhost 将在 http://localhost:4000 运行！

你还可以[将 TheGhost 连接到本地文件系统](https://docs.all-hands.dev/modules/usage/runtimes/docker#connecting-to-your-filesystem)，
以[无头模式](https://docs.all-hands.dev/modules/usage/how-to/headless-mode)运行 TheGhost，
...
启动 TheGhost 后，你**必须**选择一个 `LLM Provider` 和 `LLM Model` 并输入相应的 `API Key`。
...
TheGhost 需要 API 密钥才能访问大多数语言模型。以下是从推荐提供商获取 API 密钥的方法：
...
现在你已准备好[开始使用 TheGhost](./getting-started)。

## 版本

[上面的 docker 命令](./installation#start-the-app)会拉取 TheGhost 的最新稳定版本。你还有其他选择：
