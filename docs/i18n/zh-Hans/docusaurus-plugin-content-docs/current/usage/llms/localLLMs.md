...
使用[此处](../intro)的说明通过 Docker 启动 TheGhost。
...
现在您应该可以连接到 `http://localhost:4000/`

### 从源代码构建

使用[Development.md](https://github.com/All-Hands-AI/TheGhost/blob/main/Development.md)中的说明构建 TheGhost。
...
完成！现在您可以通过 `make run` 启动 TheGhost 而无需 Docker。现在您应该可以连接到 `http://localhost:4000/`

## 选择您的模型

在 TheGhost UI 中，点击左下角的设置齿轮。
...
WSL 中 ollama 的默认配置仅为 localhost 提供服务。这意味着您无法从 docker 容器中访问它。比如，它不会与 TheGhost 一起工作。首先让我们测试 ollama 是否正常运行。
...
```bash
docker ps # 获取正在运行的 docker 容器列表，最准确的测试选择 TheGhost sandbox 容器。
