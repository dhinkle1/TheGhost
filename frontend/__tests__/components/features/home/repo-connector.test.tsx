...
const MOCK_RESPOSITORIES: GitRepository[] = [
  ...
  {
    id: 2,
    full_name: "All-Hands-AI/TheGhost",
    git_provider: "github",
    is_public: true,
  },
];
...
beforeEach(() => {
  const getSettingsSpy = vi.spyOn(TheGhost, "getSettings");
...
describe("RepoConnector", () => {
  ...
  it("should render the available repositories in the dropdown", async () => {
    ...
    await waitFor(() => {
      ...
      screen.getByText("All-Hands-AI/TheGhost");
    });
  });
  ...
  it("should render the 'add git(hub|lab) repos' links if saas mode", async () => {
    const getConfiSpy = vi.spyOn(TheGhost, "getConfig");
  ...
  it("should create a conversation and redirect with the selected repo when pressing the launch button", async () => {
    const createConversationSpy = vi.spyOn(TheGhost, "createConversation");
    const retrieveUserGitRepositoriesSpy = vi.spyOn(
      TheGhost,
      "retrieveUserGitRepositories",
    );
